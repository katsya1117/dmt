...
// - 今は SQLite を読み書き。本番は関数のシグネチャ（引数・戻り値の型）は
//   そのまま、中身のSQL文だけを客先PHP APIへのHTTP呼び出しに差し替える
//   （呼び出し側のコントローラーは変更不要）。
...
//   【ハッシュ化をこの層に置く理由】客先PHP APIは平文ではなくハッシュ済みの
//   値を受け取る設計と判明したため、書き込み先がSQLiteからPHP呼び出しに
//   差し替わった後も、この「渡す前にハッシュ化する」処理はそのまま必要になる。
//   つまりこの層より上（コントローラー等）は今後も平文のみを扱えばよく、
//   ハッシュ化の要否・方式を意識するのはこの層だけで完結する
// ─────────────────────────────────────────────────────────────
