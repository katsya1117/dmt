...
（最終更新: 2026-07-02。tsoa化・関数名つき具体トレースを反映）
...
3. API関数   api/accountAuth.ts        fetchAccountAuthList() → fetch('/api/account-auth')
...
7. API関数     api/accountAuth.ts         !res.ok → ApiError(409, body) を throw
...
上記 4〜8（プロキシ→Express→DB）が丸ごと **MSW** に置き換わる。`fetch` のURLは同じまま `mocks/accountAuthHandlers.ts` が横取りして偽データを返す（1〜3, 9〜10は同じ）。
