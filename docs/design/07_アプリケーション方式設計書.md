...
| 層 | 役割 | 知らないこと |
|----|---|---|
| 画面 (`pages/*.tsx`) | 見た目・操作 | fetch・URL・DB |
| フック (`hooks/use*.ts`) | データ取得・キャッシュ・再取得（TanStack Query） | 画面の見た目・DB |
...
| リポジトリ (`server/src/repositories/*.ts`) | DBの読み書き。**本番化の差し替え点** | HTTP・画面 |
| DB | データ保管（デモ=SQLite、本番=客先MySQL/PHP） | 上の全部 |

```mermaid
flowchart TB
  subgraph client["クライアント (React SPA)"]
    Page["画面 pages/*.tsx"]
    Hook["フック hooks/use*.ts（TanStack Query）"]
...
- **本番化の差し替え点は`repositories/`のみ**。画面・フック・API関数・コントローラは無変更のまま、SQLiteアクセスを客先DB/PHP呼び出しに置き換える
